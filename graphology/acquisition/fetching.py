...
def process_pickles_to_tsv(timestamp: str):
    ...
    for year in range(END_YEAR, START_YEAR - 1, -1):
        ...
        for result in results:
            ...
            if result.author_ids:
                ...
                for i in range(len(ids)):
                    author_id = ids[i]
                    author_name = names[i]
                    authors[author_id] = {"author_id": author_id, "name": author_name}
        ...
        pd.DataFrame(documents).to_csv(
            RESULTS_DIRECTORY / f"documents_{year}.tsv", sep="\t", index=False
        )
        pd.DataFrame(authors.values()).to_csv(
            RESULTS_DIRECTORY / f"authors_{year}.tsv", sep="\t", index=False
        )
        pd.DataFrame(authorships).to_csv(
            RESULTS_DIRECTORY / f"authorships_{year}.tsv", sep="\t", index=False
        )
        pd.DataFrame(affiliations.values()).to_csv(
            RESULTS_DIRECTORY / f"affiliations_{year}.tsv", sep="\t", index=False
        )
